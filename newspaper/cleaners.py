...
class DocumentCleaner(object):
    def __init__(self, config):
        ...
        self.contains_article_xp = (
            './/article|.//*[@id="article"]|.//*[@itemprop="articleBody"]'
        )
        self.contains_article_tag = "article"
        self.contains_article_attrs = {"id": ["article"], "itemprop": ["articleBody"]}

    def node_contains_article(self, node):
        """
        orignal xpath(self.contains_article) doesn't work when applied directly the node
        it look only on the childs whil the tag can be on the node !
        """
        if not len(node) or node is None:
            return False
        #
        for key, value in node.items():
            if value in self.contains_article_attrs.get(key, []):
                return True
        #
        if node.xpath(self.contains_article_xp):
            return True
        #
        return False
    ...
    def clean_bad_tags(self, doc):
        ...
        for node in naughty_list:
            ...
            if not self.node_contains_article(node):
